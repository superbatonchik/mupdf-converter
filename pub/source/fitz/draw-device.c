...
void
threshold_dither(fz_pixmap * source, fz_irect bbox)
{
	int x, y;
	int startX, stopX;
	int startY, stopY;
	int stride, offset, width, height;
	unsigned char *ptr;

	//Threshold now to prevent messing this region up with another pass
	stride = source->w * 2;//Because we have an alpha channel
	width = (bbox.x1 - bbox.x0);
	height = (bbox.y1 - bbox.y0);
	offset = stride - (width * 2);
	startY = bbox.y0 - source->y;
	startX = bbox.x0 - source->x;
	stopY = startY + height;
	stopX = startX + width;
		
	ptr = source->samples;
	ptr += ((startY * stride) + (startX * 2));
	for (y = startY; y < stopY; y++)
	{
		for(x = startX; x < stopX; x++, ptr += 2)
		{
			//Just set the bits.  Faster in testing than checking first.
...
void
error_diffuse(fz_pixmap *source, fz_irect box)
{
	int y, x, v, error, ed;
	int startX, stopX;
	int startY, stopY;
	...
	offset = stride - (width * 2);
	startY = box.y0 - source->y;
	startX = box.x0 - source->x;
	stopY = startY + height;
	stopX = startX + width;
	...
	for (y = startY; y < stopY; y++)
    {
		for(x = startX; x < stopX; x++, ptr += 2)
		{
			...
			if(x + 1 < stopX)
			{
				ed = (float)ptr[2] + ((float)error * 7.0f) / 16.0f;
			...
			if(y + 1 < stopY)
			{
				...
				if(x - 1 >= 0)
				{
					ed = ptr2[-2] + (error * 3) / 16;
					ed = (ed < 0) ? 0 : ((ed > 255) ? 255 : ed);
					ptr2[-2] = ed;
				}

				ed = ptr2[0] + (error * 5) / 16;
				ed = (ed < 0) ? 0 : ((ed > 255) ? 255 : ed);
				ptr2[0] = ed;
	...
	fz_colorspace *colorspace, float *color, float alpha)
{
	...
	//Error diffuse shading if threshold will turn it black.  Otherwise let it go white for b/w.
	//Need to apply some dithering here.
	if(model == fz_device_mono(dev->ctx) && colorbv[0] != 0 && colorbv[0] != 255)
	{
		//If original colorspace is CMYK we may have issues with getting true black or true white.
		//Clip the bottom 18%
		if(colorspace == fz_device_cmyk(dev->ctx) && (colorbv[0] < 179 && colorbv[0] > 46))
			error_diffuse(state->dest, bbox);
		else if(colorspace != fz_device_cmyk(dev->ctx) && colorbv[0] < 179)
			error_diffuse(state->dest, bbox);
		else
			threshold_dither(state->dest, bbox);
	}
	...
	fz_colorspace *colorspace, float *color, float alpha)
{
	...
	fz_convert_color(dev->ctx, model, colorfv, colorspace, color);
	if(model == fz_device_mono(dev->ctx) && colorfv[0] < 0.80f)
		colorfv[0] = 0.0f;//This will force black for mono
	for (i = 0; i < model->n; i++)
		colorbv[i] = colorfv[i] * 255;
	colorbv[i] = alpha * 255;
...
fz_draw_fill_image(fz_device *devp, fz_image *image, const fz_matrix *ctm, float alpha)
{
	...
	if(model == fz_device_mono(ctx) && (image->colorspace == fz_device_gray(ctx) || image->colorspace == fz_device_mono(ctx)))
	{
		//Check to see if original image is black/white
		BlackWhite = 1;

		length = pixmap->w * pixmap->h * pixmap->n;
		output = pixmap->samples;
	
		for(k = 0; k < length; k += 2)//Every 2nd byte since we have an alpha channel
		{
			if(output[k] != 0 && output[k] != 255)
			{
				BlackWhite = 0;
				break;
			}
	...
	fz_try(ctx)
	{
		...
		after = 0;
		if (pixmap->colorspace == fz_device_gray(ctx) || pixmap->colorspace == fz_device_mono(ctx))
			after = 1;
		...
		if(model != fz_device_mono(dev->ctx))
		{
		...
		//Paint image first and then do error diffusion!
		if(model == fz_device_mono(ctx))
		{
			...
			if(!BlackWhite)
			{
				...
				stride = state->dest->w * 2;//Because we have an alpha channel
				width = (state->scissor.x1 - state->scissor.x0);
				height = (state->scissor.y1 - state->scissor.y0);
				offset = stride - (width * 2);
				startY = state->scissor.y0 - state->dest->y;
				startX = state->scissor.x0 - state->dest->x;
				stopY = startY + height;
				stopX = startX + width;

				ptr = state->dest->samples;
				ptr += ((startY * stride) + (startX * 2));
				for (y = startY; y < stopY; y++)
				{
					if(ErrorDiffusion)
					...
					for(x = startX; x < stopX; x++, ptr += 2)
					{
						if(!ErrorDiffusion && ptr[0] > 46 && ptr[0] < 209)
						{
							ErrorDiffusion = 1;
							break;
						}
				...
				//Need to apply some dithering here.  Problem is we have alpha channel we have to ignore.
				if(ErrorDiffusion)
				{
					error_diffuse(state->dest, state->scissor);
					...
					//	for(x = startX; x < stopX; x++, ptr += 2)
					//	{
					//		if(!ErrorDiffusion && ptr[0] != 0 && ptr[0] != 255)
					//		{
					//			ErrorDiffusion = 1;
					//			break;
					//		}
			...
			{
				//Threshold now to prevent messing this region up with another pass
				threshold_dither(state->dest, state->scissor);
			}
		}
		if (state->blendmode & FZ_BLEND_KNOCKOUT)
			fz_knockout_end(dev);
	...
	fz_colorspace *colorspace, float *color, float alpha)
{
	...
	fz_try(ctx)
	{
		...
		{
			BlackWhite = 1;
			if(colorbv[0] != 0 && colorbv[0] != 255)
			{
				//This is a cheap way to get black when threshold takes place.
				//Probably prefer error diffusion since technically we are working with images.
				/*if(colorbv[0] < 205)
					colorbv[0] = 0;
				else
					BlackWhite = 0;*/
				...
				//If original colorspace is CMYK we may have issues with getting true black or true white.
				//Clip the top/bottom 10%
				if(colorspace == fz_device_cmyk(dev->ctx) && (colorbv[0] < 25))
					colorbv[0] = 0;
				else if(colorspace == fz_device_cmyk(dev->ctx) && (colorbv[0] > 230))
					colorbv[0] = 255;
				else
					BlackWhite = 0;
			
				if(colorbv[0] != 0 && colorbv[0] < 205)//Darken color to improve readability since this is usually signatures and misc stuff on the doc.
					colorbv[0] = colorbv[0] * 0.5f;
			}
		}
		else
			BlackWhite = 0;
		...
		//Paint image first and then do error diffusion!
		//This may get called a few times for the same region.  Although testing proves no significant increase in overall rendering time.
		if(model == fz_device_mono(dev->ctx) && colorbv[0] != 0 && colorbv[0] != 255)
		{
			fz_irect bbox;
			fz_rect urect;
			...
			if(!BlackWhite)
			{
				...
				startY = bbox.y0 - state->dest->y;
				startX = bbox.x0 - state->dest->x;
				stopY = startY + height;
				stopX = startX + width;
			...
			if(!ErrorDiffusion)
			{
				//Threshold now to prevent messing this region up with another pass later
				threshold_dither(state->dest, bbox);
			}
		}
		if (scaled)
