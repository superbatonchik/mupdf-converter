...
static void rgb_to_gray(fz_context *ctx, fz_colorspace *cs, float *rgb, float *gray)
{
	...
	float g = rgb[1];
	float b = rgb[2];
	gray[0] = r * 0.299f + g * 0.587f + b * 0.114f;
}
...
static fz_colorspace k_default_mono = { {-1, fz_free_colorspace_imp}, 0, "DeviceMono", 1, gray_to_rgb, rgb_to_gray };
...
static fz_colorspace *fz_default_mono = &k_default_mono;
...
{
	int ctx_refs;
	fz_colorspace *mono, *gray, *rgb, *bgr, *cmyk;
};
...
void fz_new_colorspace_context(fz_context *ctx)
{
	...
	ctx->colorspace->mono = fz_default_mono;
...
fz_colorspace *
fz_device_mono(fz_context *ctx)
{
	return ctx->colorspace->mono;
}
...
{
	if (!strcmp(name, "DeviceMono"))
		return fz_device_mono(ctx);
...
void
fz_set_device_mono(fz_context *ctx, fz_colorspace *cs)
{
	fz_drop_colorspace(ctx, ctx->colorspace->mono);
	ctx->colorspace->mono = fz_keep_colorspace(ctx, cs);
}
...
static void fast_cmyk_to_gray(fz_pixmap *dst, fz_pixmap *src)
{
	...
	{
#ifdef SLOWCMYK
		float cmyk[4], rgb[3], gray[1];
		cmyk[0] = s[0] / 255.0f;
		cmyk[1] = s[1] / 255.0f;
		cmyk[2] = s[2] / 255.0f;
		cmyk[3] = s[3] / 255.0f;
		cmyk_to_rgb(NULL, NULL, cmyk, rgb);
		rgb_to_gray(NULL, NULL, rgb, gray);
		d[0] = gray[0] * 255;
#else
...
fz_convert_pixmap(fz_context *ctx, fz_pixmap *dp, fz_pixmap *sp)
{
	...
	if (ss == fz_default_gray || ss == fz_default_mono)
	{
	...
	{
		if (ds == fz_default_gray || ds == fz_default_mono) fast_rgb_to_gray(dp, sp);
	...
	{
		if (ds == fz_default_gray || ds == fz_default_mono) fast_bgr_to_gray(dp, sp);
	...
	{
		if (ds == fz_default_gray || ds == fz_default_mono) fast_cmyk_to_gray(dp, sp);
...
{
	dv[0] = sv[0] * 0.114f + sv[1] * 0.587f + sv[2] * 0.299f;
}
...
#ifdef SLOWCMYK
		float rgb[3];
		cmyk_to_rgb(NULL, NULL, sv, rgb);
		rgb_to_gray(NULL, NULL, rgb, dv);
#else
		float c = sv[0] * 0.299f;
		float m = sv[1] * 0.587f;
		float y = sv[2] * 0.114f;
...
void fz_lookup_color_converter(fz_color_converter *cc, fz_context *ctx, fz_colorspace *ds, fz_colorspace *ss)
{
	...
	{
		if (ds == fz_default_gray || ds == fz_default_mono)
			cc->convert = rgb2g;
