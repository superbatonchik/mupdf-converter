...
int MuPDFClass::LoadPdfInit(char* password)
{
	...
	_PageCount = fz_count_pages(_doc);
...
int MuPDFClass::LoadPage(int pageNumber)
{
	...
	fz_try(_ctx)
	{
		_page = fz_load_page(_doc, pageNumber - 1);
		fz_bound_page(_doc, _page, &bounds);
...
unsigned char* MuPDFClass::GetBitmap(int & width, int & height, float dpiX, float dpiY, int rotation, int colorspace, bool rotateLandscapePages, bool convertToLetter, int & pnLength, int maxSize)
{
	...
	fz_pre_scale(fz_rotate(&ctm, rotation),zoomX, zoomY);
	tbounds = bounds;
	...
	fz_try(_ctx)
	{
		pix = fz_new_pixmap_with_bbox(_ctx, destcolor, &ibounds);
		if (!pix)
			return NULL;
		pix->xres = dpiX;
		pix->yres = dpiY;
		...
		fz_run_page(_doc, _page, dev, &ctm, NULL);
